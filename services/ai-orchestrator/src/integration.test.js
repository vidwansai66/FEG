...
describe("Live Integration Tests against Member 3 (localhost:3000)", () => {
    it("should perform a full end-to-end resolution for an event", async () => {
        ...
        const event = events.find((e) => e.label.toLowerCase().includes("arsenal"));
        ...
        const market = markets.find((m) => m.label.includes("Match Result"));
        ...
        const selection = selectionsResult.candidates.find((s) => s.label.toLowerCase().includes("arsenal"));
