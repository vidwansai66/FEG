...
// ===========================================================================
export const mockSearchEvents = async (input) => {
    const q = input.query.toLowerCase();
    if (q.includes("error_event"))
        throw new Error("Simulated event error");
    if (q.includes("timeout_event"))
        await new Promise(resolve => setTimeout(resolve, 5000));
    if (q.includes("malformed_event"))
        return [{ invalid: "data" }];
...
export const mockSearchMarkets = async (input) => {
    const q = input.query?.toLowerCase() || "";
    if (q.includes("error_market"))
        throw new Error("Simulated market error");
    if (q.includes("timeout_market"))
        await new Promise(resolve => setTimeout(resolve, 5000));
    if (q.includes("malformed_market"))
        return [{ invalid: "data" }];
...
export const mockResolveSelection = async (input) => {
    const q = input.selectionQuery.toLowerCase();
    if (q.includes("error_selection"))
        throw new Error("Simulated selection error");
    if (q.includes("timeout_selection"))
        await new Promise(resolve => setTimeout(resolve, 5000));
    if (q.includes("malformed_selection"))
        return { invalid: "data" };
    if (q.includes("odds_fail")) {
        return { candidates: [{ id: "error_odds", label: "Error Odds", type: "selection", confidence: 1.0, matchReason: "exact", item: {} }] };
    }
    if (q.includes("odds_retry")) {
        return { candidates: [{ id: "retry_odds", label: "Retry Odds", type: "selection", confidence: 1.0, matchReason: "exact", item: {} }] };
    }
...
export const mockGetCurrentOdds = async (input) => {
    if (input.selectionIds.includes("error_odds"))
        throw new Error("Simulated odds error");
    if (input.selectionIds.includes("timeout_odds"))
        await new Promise(resolve => setTimeout(resolve, 5000));
    if (input.selectionIds.includes("malformed_odds"))
        return [{ invalid: "data" }];
