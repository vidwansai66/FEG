import { UserIntentSchema, UserIntent } from './schemas.js';
import { SYSTEM_PROMPT_INTENT_EXTRACTION } from './prompts.js';
import { createDraftSlip, updateDraftSlip, validateDraftSlip, BackendSlipLeg } from './backendClient.js';

export async function syncWithMember4(
  sessionId: string, 
  intent: UserIntent, 
  legs: ProcessedLeg[], 
  odds: ResolvedOdds[], 
  ...
  const backendLegs: BackendSlipLeg[] = legs.map((leg, i) => {
    const odd = odds.find(o => o.selectionId === leg.selection.id);
    if (!odd) throw new Error(`Missing odds for selection: ${leg.selection.id}`);
    return {
 ...
 */
export async function extractUserIntent(userInput: string): Promise<UserIntent> {
  ...
  while (attempt <= maxRetries) {
    try {
      ...
      if (attempt === maxRetries) {
        if (isTimeout) throw new Error(`[${operationName}] Timeout after ${maxRetries} retries`);
        if (isMalformed) throw new Error(`[${operationName}] Malformed response`);
...
export type EventResolutionResult = 
  | { status: "SUCCESS"; event: ResolvedEvent }
  | { status: "NOT_FOUND"; message: string }
  | { status: "NEEDS_CLARIFICATION"; candidates: ResolvedEvent[] }
  | { status: "TOOL_ERROR"; message: string; isTimeout: boolean };

/**
 * Executes the first orchestration step: resolving an eventQuery to a verified eventId.
 */
export async function resolveEventIntent(query: string): Promise<EventResolutionResult> {
...
export type MarketResolutionResult = 
  ...
  | { status: "NOT_FOUND"; message: string }
  | { status: "NEEDS_CLARIFICATION"; candidates: ResolvedMarket[] }
  | { status: "TOOL_ERROR"; message: string; isTimeout: boolean };

/**
 * Executes the second orchestration step: resolving a marketQuery given a verified eventId.
 */
export async function resolveMarketIntent(eventId: string, marketQuery: string): Promise<MarketResolutionResult> {
...
export type SelectionResolutionResult = 
  ...
  | { status: "NOT_FOUND"; message: string }
  | { status: "NEEDS_CLARIFICATION"; candidates: ResolvedSelectionCandidate[] }
  | { status: "TOOL_ERROR"; message: string; isTimeout: boolean };
 ...
 */
export async function resolveOddsIntent(selectionIds: string[]): Promise<OddsResolutionResult> {
...
export type OrchestrationResult = 
  | { status: "SUCCESS"; stake?: number; legs: ProcessedLeg[]; odds: ResolvedOdds[] }
  | { status: "NOT_FOUND"; legIndex: number; step: "event" | "market" | "selection"; message: string }
  | { status: "NEEDS_CLARIFICATION"; legIndex: number; step: "event" | "market" | "selection"; candidates: ClarificationCandidate[]; cachedLegs: PartialLeg[] }
  | { status: "TOOL_ERROR"; legIndex?: number; step?: "event" | "market" | "selection" | "odds"; message: string; isTimeout?: boolean }
  | { status: "ODDS_FAILED"; message: string }
  | { status: "CONFIRMATION_REQUIRED"; slipId: string; totalOdds: number; draft: { stake?: number; legs: ProcessedLeg[]; odds: ResolvedOdds[] } }
  | { status: "INVALID_SLIP"; slipId: string; issues: any[] }
  | { status: "CLARIFICATION_ERROR"; message: string }
  | { status: "EDIT_ERROR"; message: string };
...
export async function processUserIntent(
  ...
  const processedLegs: ProcessedLeg[] = [];
...
export async function processUserMessage(
  ...
  message: string, 
  intent?: UserIntent
): Promise<OrchestrationResult | { status: "CLARIFICATION_ERROR", message: string }> {
  ...
  if (session.pendingClarification) {
    ...
    let selected: ClarificationCandidate | undefined;
    ...
    if (step === "event") currentLeg.event = selected as ResolvedEvent;
    else if (step === "market") currentLeg.market = selected as ResolvedMarket;
    else if (step === "selection") currentLeg.selection = selected as ResolvedSelectionCandidate;
    ...
    if (result.status === "NEEDS_CLARIFICATION") {
       ...
       if ("slipId" in finalResult) session.activeSlipId = finalResult.slipId;
...
// ===========================================================================
// ORCHESTRATION: EDIT COMMANDS & CONFIRMATION
// ===========================================================================

export type EditIntent = 
  | { action: "edit_stake"; stake: number }
  | { action: "remove_leg"; legIndex?: number; targetQuery?: string } // 1-based index or string search
  | { action: "change_selection"; legIndex?: number; targetQuery?: string; newSelectionQuery: string };

export type ConfirmIntent = { action: "confirm_slip" };

export async function processConfirmIntent(sessionId: string): Promise<OrchestrationResult> {
  const session = getSession(sessionId);
  ...
  return session.lastResult as OrchestrationResult;
}
...
export async function processEditIntent(
  sessionId: string, 
  edit: EditIntent
): Promise<OrchestrationResult | { status: "EDIT_ERROR"; message: string }> {
  const session = getSession(sessionId);
  ...
  const newIntent: UserIntent = JSON.parse(JSON.stringify(session.activeIntent));
  const newCachedLegs = [...session.activeLegs];

  if (edit.action === "edit_stake") {
    newIntent.stake = edit.stake;
    const finalResult = await syncWithMember4(
      sessionId, 
      newIntent, 
      session.lastResult.draft?.legs || session.lastResult.legs, 
      session.lastResult.draft?.odds || session.lastResult.odds, 
      session.activeSlipId
    );
    if ("slipId" in finalResult) session.activeSlipId = finalResult.slipId;
...
function findLegIndex(legs: PartialLeg[], query: string): number {
